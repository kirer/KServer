        ...
        switch (msg->type) {
            case MSG_TYPE_INIT_REQ: {
                ...
                init_response_data_t response = {
                    .shm_size = g_server.config.shm_size,
                    .pixel_format = PIXEL_FORMAT_RGBA_8888,
                    .server_version = PROTOCOL_VERSION,
                    .reserved = 0
                };

                if (server_send_message(client_fd, MSG_TYPE_INIT_RESP, &response, sizeof(response)) < 0) {
            ...
            case MSG_TYPE_SCREENSHOT_ACK: {
                LOG_DEBUG(SERVER_LOG_TAG, "收到截图确认");
                break;
            }
            ...
            default:
                LOG_WARN(SERVER_LOG_TAG, "收到未知消息类型: %d", msg->type);
                break;
        }
...
int server_init(const server_config_t *config) {
    ...
    LOG_INFO(SERVER_LOG_TAG, "初始化服务器");
    // 清理现有状态
    server_cleanup();
    // 复制配置
...
int server_start(void) {
    ...
    if (server_parse_address(g_server.config.address, host, &port, socket_name, g_server.config.socket_type) < 0) {
    ...
    // 创建监听socket
    if (g_server.config.socket_type == SOCKET_TYPE_TCP) {
        g_server.listen_fd = server_create_tcp_socket(host, port);
    } else if (g_server.config.socket_type == SOCKET_TYPE_UNIX) {
...
int server_notify_screenshot(uint64_t timestamp, uint32_t data_size) {
    ...
    screenshot_notify_data_t notify_data = {
        .timestamp = timestamp,
        .data_size = data_size,
        .reserved = 0
    };
    ...
    for (int i = 0; i < g_server.client_count; i++) {
        if (g_server.clients[i].active) {
            if (server_send_message(g_server.clients[i].socket_fd, MSG_TYPE_SCREENSHOT_NOTIFY, 
                                  &notify_data, sizeof(notify_data)) == 0) {
                success_count++;
            } else {
    ...
    LOG_DEBUG(SERVER_LOG_TAG, "通知 %d 个客户端新截图可用", success_count);
...
int server_write_screenshot(const void *data, size_t size) {
    ...
    // 写入共享内存
    if (shm_write(data, size) < 0) {
        LOG_ERROR(SERVER_LOG_TAG, "写入共享内存失败");
        return -1;
    }
    
    // 通知客户端
    uint64_t timestamp = shm_get_timestamp();
    return server_notify_screenshot(timestamp, (uint32_t)size);
}
...
Java_com_github_kirer_server_Server_initialize(JNIEnv *env, jclass clazz,
                                               jint socket_type, jstring address, jint shm_size, jboolean debug) {
...
Java_com_github_kirer_server_Server_writeScreenshot(JNIEnv *env, jclass clazz, jbyteArray data) {
    ...
    int result = server_write_screenshot(bytes, (size_t)len);
