    ...
    try {
        ...
        if (!img.empty()) {
            ...
            if (imwrite(filename, img)) {
                ...
                LOG_ERROR(MAIN_LOG_TAG, "保存截图失败: %s", filename);
            }
        } else {
...
void on_screenshot(const uint8_t *data, size_t size, uint64_t timestamp, void *user_data) {
    g_screenshot_count++;
    LOG_INFO(MAIN_LOG_TAG, "收到截图 #%d: 大小=%zu 字节, 时间戳=%llu", 
             g_screenshot_count, size, timestamp);
    
    // 保存截图到文件
    save_screenshot_to_file(data, size, timestamp);
}
...
void on_error(int error_code, const char *message, void *user_data) {
    LOG_ERROR(MAIN_LOG_TAG, "服务器错误: 代码=%d, 消息=%s", error_code, message);
}
 ...
 * 启动k-server
 */
static int start_k_server(socket_type_t socket_type, const char* address) {
    ...
    if (socket_type == SOCKET_TYPE_TCP) {
        snprintf(command, sizeof(command),
            ...
            dex_path, mode_str, so_path, host, port,
            g_client.config.debug ? "--debug" : "");
    } else {
        snprintf(command, sizeof(command),
            "CLASSPATH=%s app_process /system/bin com.github.kirer.server.Launcher "
            "--mode=%s --libPath=%s --socketName=%s %s > /data/local/tmp/k-server.log 2>&1 &",
            dex_path, mode_str, so_path, socket_name,
            g_client.config.debug ? "--debug" : "");
    }
...
int client_auto_start_server(void) {
    if (!g_client.config.auto_start_server) {
        return 0; // 未配置自动启动
    }
    ...
    // 启动新的服务器
    if (start_k_server(g_client.config.socket_type, g_client.config.address) < 0) {
...
int main(int argc, char *argv[]) {
    ...
    LOG_INFO(MAIN_LOG_TAG, "Socket类型: %s", config.socket_type == SOCKET_TYPE_TCP ? "TCP" : "UNIX");
    LOG_INFO(MAIN_LOG_TAG, "服务器地址: %s", config.address);
    LOG_INFO(MAIN_LOG_TAG, "自动启动服务器: %s", config.auto_start_server ? "是" : "否");
