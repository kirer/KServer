...
static int client_handle_init_response(const control_message_t *msg) {
    ...
    // 保存服务器信息
    g_client.server_info.shm_size = response->shm_size;
    g_client.server_info.pixel_format = response->pixel_format;
    g_client.server_info.server_version = response->server_version;
    
    LOG_INFO(CLIENT_LOG_TAG, "收到服务器信息: %dx%d, 共享内存: %u 字节", response->shm_size);
    
    // 连接共享内存
    if (shm_connect(response->shm_size - 16) < 0) {
        LOG_ERROR(CLIENT_LOG_TAG, "连接共享内存失败");
        return -1;
    }
...
static void client_handle_screenshot_notify(const control_message_t *msg) {
    if (!msg || msg->data_size != sizeof(screenshot_notify_data_t)) {
        LOG_WARN(CLIENT_LOG_TAG, "截图通知格式错误");
        return;
    }
    
    const screenshot_notify_data_t *notify = (const screenshot_notify_data_t*)msg->data;
    
    LOG_DEBUG(CLIENT_LOG_TAG, "收到截图通知: 时间戳=%llu, 大小=%u", notify->timestamp, notify->data_size);
    
    // 如果设置了回调函数，读取数据并调用回调
    if (g_client.on_screenshot) {
        uint8_t *data = NULL;
        size_t size = 0;
        
        if (shm_read(&data, &size) == 0 && data) {
            g_client.on_screenshot(data, size, notify->timestamp, g_client.user_data);
            free(data);
        }
    }
}
...
static void client_handle_error_message(const control_message_t *msg) {
    if (!msg || msg->data_size < sizeof(error_message_data_t)) {
        LOG_WARN(CLIENT_LOG_TAG, "错误消息格式错误");
        return;
    }
    
    const error_message_data_t *error = (const error_message_data_t*)msg->data;
    
    // 提取错误消息字符串
    char error_msg[256] = {0};
    if (error->message_len > 0 && error->message_len < sizeof(error_msg)) {
        memcpy(error_msg, error->message, error->message_len);
        error_msg[error->message_len] = '\0';
    }
    
    LOG_ERROR(CLIENT_LOG_TAG, "收到服务器错误: 代码=%u, 消息=%s", error->error_code, error_msg);
    
    // 如果设置了错误回调函数，调用它
    if (g_client.on_error) {
        g_client.on_error(error->error_code, error_msg, g_client.user_data);
    }
}
...
static void* client_receive_thread(void *arg) {
    ...
    while (!g_client.should_stop && g_client.state == CLIENT_STATE_CONNECTED) {
        ...
        switch (msg->type) {
            case MSG_TYPE_SCREENSHOT_NOTIFY:
                client_handle_screenshot_notify(msg);
                break;
                
            case MSG_TYPE_ERROR:
                client_handle_error_message(msg);
                break;
            ...
            default:
                LOG_WARN(CLIENT_LOG_TAG, "收到未知消息类型: %d", msg->type);
                break;
        }
...
static int client_connect_socket(void) {
    ...
    if (client_parse_address(g_client.config.address, host, &port, socket_name, g_client.config.socket_type) < 0) {
...
int client_init(const client_config_t *config) {
    ...
    LOG_INFO(CLIENT_LOG_TAG, "初始化客户端");
    ...
    g_client.on_screenshot = NULL;
    g_client.on_error = NULL;
    g_client.user_data = NULL;
    
    // 清空服务器信息
    memset(&g_client.server_info, 0, sizeof(g_client.server_info));
...
void client_set_screenshot_callback(void (*callback)(const uint8_t *data, size_t size, uint64_t timestamp, void *user_data), void *user_data) {
    g_client.on_screenshot = callback;
    g_client.user_data = user_data;
}
...
int client_read_screenshot(uint8_t **data, size_t *size, uint64_t *timestamp) {
    if (g_client.state != CLIENT_STATE_CONNECTED || !data || !size) {
        return -1;
    }
    
    int result = shm_read(data, size);
    if (result == 0 && timestamp) {
        *timestamp = shm_get_timestamp();
    }
...
Java_com_github_kirer_server_Client_readScreenshot(JNIEnv *env, jclass clazz) {
    ...
    int result = client_read_screenshot(&data, &size, &timestamp);
    if (result != 0 || !data) {
        return NULL;
    }
    ...
    if (array) {
        (*env)->SetByteArrayRegion(env, array, 0, (jsize)size, (jbyte *)data);
    }
    free(data);
    return array;
}
